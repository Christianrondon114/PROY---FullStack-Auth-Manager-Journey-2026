...
public class ShoppingCartController {
    ...
    public ResponseEntity<ShoppingCartResponse> addToCart(@AuthenticationPrincipal UserDetails userDetails, @RequestBody AddToCartRequest request) {
        ShoppingCartResponse response = shoppingCartService.addToCart(userDetails, request);
        return ResponseEntity.status(HttpStatus.OK).body(response);
    }
